...
function Main_parent(props) {
  ...
  const [userlongitude, setuserlongitude]=useState( 35.495480);
  const [userLatitude, setuserLatitude]=useState( 33.888630);
...
axios.defaults.baseURL="";
  ...
  function GetUserProfile() {
    ...
    axios.get(process.env.REACT_APP_API_URL+"/getParentDetails")
          .then((response)=>{
            let usersarray=[];
            setuserProfile([]);
              usersarray.push({
    ...
    function getbabysitters(){
      let usersarray=[]
      setbabysitterslist([])
      axios.get(process.env.REACT_APP_API_URL + '/getBabysittersinCity/'+ userCity_ID)
      .then((response)=>{ 
        //console.log(response);  
    ...
    function getbabysitterstoMap(){
      ...
      axios.get(process.env.REACT_APP_API_URL+`/getAllBabysitters`)
      .then((response)=>{ 
        //console.log(response);  
        console.log(response.data[0]);   
        for(let i=0;i<response.data.length;i++){
    ...
    function GetBabysitterInfo() {
      ...
      axios.get(process.env.REACT_APP_API_URL+"/babysitterDetails/"+chosenBabysitter)
            .then((response)=>{
              console.log("Chosen babysitter"+chosenBabysitter)
              console.log(response);
              let usersarray=[];
              setbabysitterInfo([]);
                usersarray.push({
      ...
      function getAppointments(){
        ...
        setAppointmentLast([]);
        if (status===0){
        axios.get(process.env.REACT_APP_API_URL + `/getPending`)
        .then((response)=>{ 
          //console.log(response);  
          console.log(response);   
         for(let i=0;i<response.data.length;i++){
    ...
    axios.get(process.env.REACT_APP_API_URL+`/getScheduled`)
        .then((response)=>{ 
          //console.log(response);  
          console.log(response);   
         for(let i=0;i<response.data.length;i++){
...
<>
    <CssBaseline />
    <Header showAppointment={true} setOpenChat={setOpenChat} setopenProfile={setopenProfile} setopenAppointmentsList={setopenAppointmentsList} getAppointments={getAppointments} notificationList={notificationList} NotificationRead={NotificationRead} notificationCount={notificationCount} loadNotifcations={loadNotifcations} getNotification={getNotification}/>
    ...
    <TextField
      ...
      style={{marginBottom:'15px', background:"white", border:"none", marginTop:"5px"}}
      InputProps={{
        endAdornment:(
    ...
    />
      ...
      <ListMUI sx={{ width: '100%', bgcolor: 'background.paper' }}>
        {listloaded===false || SearchLoading===true? <h5 style={{textAlign: 'center'}}>loading...</h5>:(babysitterslist.length!==0?
        <List babysitterlist={babysitterslist} openprofile={setopenBabysitterProfile} setchosenBabysitter={setchosenBabysitter}/> :
    ...
    <Map  locationLat={userLatitude} locationLong={userlongitude} babysitterList={babysitterListMap}/>:
    <div>Loading</div>}
        <Fab variant="extended" size="medium" color="primary" onClick={(e)=>{setopenfeelinglucky(true)}}
  ...
  Feeling Lucky
    ...
    <BabysitterProfile openbooking={setopenbookApp} babysitterInfo={babysitterInfo} loadingInfo={loadingInfo} setopenRate={setopenRate}/> :
    <div>loading...</div>}
    ...
    >
      <Appointments_list status={status} setStatus={setstatus} AppointmentList={AppointmentList} ListLoaded={AppListLoaded} getAppointments={getAppointments} />
