...
function Main_parent(props) {
  ...
  function GetUserProfile() {
    axios
      .get(process.env.REACT_APP_API_URL + "/babysitter/getDetails")
      .then((response) => {
        console.log(response);
        let usersarray = [];
        setuserProfile([]);
  ...
  function getAppointments() {
    ...
    if (status === 0) {
      axios
        .get(process.env.REACT_APP_API_URL + `/babysitter/getAppointments`)
        .then((response) => {
          //console.log(response);
          console.log(response);
          for (let i = 0; i < response.data.length; i++) {
      ...
      axios
        .get(process.env.REACT_APP_API_URL + `/babysitter/getScheduled`)
        .then((response) => {
          //console.log(response);
          console.log(response);
          for (let i = 0; i < response.data.length; i++) {
  ...
  return (
    <>
            ...
            >
              {appointmentlistLoaded ? (
                pendingAppointmentList.length > 0 ? (
                  <AppointmentList
                    PendingList={pendingAppointmentList}
                    setChosen={setchosenAppointmnet}
                    error={seterror}
                    success={setsuccess}
                    chosenAppointmnet={chosenAppointmnet}
                  />
                ) : (
      ...
      <Popup title="Chat" openPopup={openChat} setOpenPopup={setOpenChat}>
        <Chatting
          user={userProfile}
          setMessagesCount={setMessagesCount}
          messagesCount={messagesCount}
