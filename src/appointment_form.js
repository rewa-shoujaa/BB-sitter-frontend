...
export default function Appointment(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if (
      ...
      console.log(
        "error start and end time should be greater than the current date/time"
      );
      seterror(
        "Start and end date/time should be greater than the current date/time"
      );
      setsuccess(null);
    } else if (startDate > endDate) {
      console.log("error ");
      seterror("End date/time should not be greater than start date/time ");
      setsuccess(null);
    } else if (
      Moment(endDate, "YYYY-MM-DD[T]hh:mm").diff(
        Moment(startDate, "YYYY-MM-DD[T]hh:mm"),
        "hours"
      ) > 8
    ) {
      ...
      axios
        .post(process.env.REACT_APP_API_URL + "/BookAppointment", {
